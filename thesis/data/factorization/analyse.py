...
numbers = np.loadtxt("data/factorization/numbers.txt", dtype=np.int64)
...
for i in range(0, numbers.shape[0]):
    _run, _success, _time, _steps = np.loadtxt("data/factorization/data/"+str(i)+".txt", unpack=True, dtype=np.int64)
...
np.savetxt("data/factorization/results.txt", np.c_[n, success, successError, time, timeError, steps, stepsError, timeClean, timeErrorClean, stepsClean, stepsErrorClean], fmt="%4f")
