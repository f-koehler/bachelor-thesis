...
for i in range(0, numbers.shape[0]):
    _run, _success, _time, _steps = np.loadtxt("data/runtime/data/"+str(i)+".txt", unpack=True, dtype=np.int64)
...
np.savetxt("data/runtime/results.txt", np.c_[n, success, successError, time, timeError, steps, stepsError, timeClean, timeErrorClean, stepsClean, stepsErrorClean], fmt="%4f")
